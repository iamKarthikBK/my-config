    ...
    def _monitor_recv(self):
        ...
        while True:
            ...
            if self.en.value.integer == 1:
                ...
                vec = (self.instr_f7.value.integer,
                       self.instr_f3.value.integer, self.f5_valid.value,
                       self.instr_f5.value.integer, self.op1.value.integer,
...
class OMonitor(Monitor):
    ...
    def __init__(self, dut, callback=None, event=None, enlog=False):
        ...
        self.log = logging.getLogger("cocotb")
        self.enlog = enlog
        self.xlen = dut.ma_inputs_rs1.value.n_bits
        Monitor.__init__(self, callback, event)
    ...
    def _monitor_recv(self):
        ...
        while True:
            ...
            # trigger sample inputs when the operation is detected
            if (self.resp_rdy.value.integer == 1):
                ...
                recv_val = self.resp.value.integer
                self._recv(recv_val)
                if self.enlog:
