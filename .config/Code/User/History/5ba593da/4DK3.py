...
class Checker:
    ...
    def run_ref_model(self, privkey):
        try:
            return PubKeyGen(privkey)
        except Exception:
            pass
...
class IMonitor(Monitor):
    ...
    def _monitor_recv(self):
        ...
        while True:
            ...
            if self.en_priv_key.value.integer == 1:
...
class OMonitor(Monitor):
    ...
    # is the sub-module name (without hierarchy) and the next argument is the
    # category : inputs, outputs, registers.
    _signals = get_signals('mk_pubkey_gen', 'outputs')
    ...
    def _monitor_recv(self):
        ...
        while True:
            ...
            # trigger sample inputs when the operation is detected
            if (self.pub_key_rdy_fst.value.integer == 1 and self.pub_key_rdy_snd.value.integer == 1):
                ...
                recv_val = (self.pub_key_fst.value.integer, self.pub_key_snd.value.integer)
                self._recv(recv_val)
                if self.enlog:
                    self.log.debug(f'crypto-accel.omon: \nPx = {hex(recv_val[0])}\nPy = {}'.format(hex(recv_val[1])))')
